...
Matcher::Matcher(parameters param) : _param(param)
{
    ...
    _I1c_du = 0; _I1c_dv = 0;
    _I2c_du = 0; _I2c_dv = 0;
    _I1p_du_full = 0; _I1p_dv_full = 0;
    _I2p_du_full = 0; _I2p_dv_full = 0;
    _I1c_du_full = 0; _I1c_dv_full = 0;
    _I2c_du_full = 0; _I2c_dv_full = 0;
...
Matcher::~Matcher()
{
    if (_I1p)          _mm_free(_I1p);
    if (_I2p)          _mm_free(_I2p);
    if (_I1c)          _mm_free(_I1c);
    if (_I2c)          _mm_free(_I2c);
    if (_m1p1)         _mm_free(_m1p1);
    if (_m1p2)         _mm_free(_m1p2);
    if (_I1p_du)       _mm_free(_I1p_du);
    if (_I1p_dv)       _mm_free(_I1p_dv);
    if (_I1p_du_full)  _mm_free(_I1p_du_full);
    if (_I1p_dv_full)  _mm_free(_I1p_dv_full);
    if (_m2p1)         _mm_free(_m2p1);
    if (_m2p2)         _mm_free(_m2p2);
    if (_I2p_du)       _mm_free(_I2p_du);
    if (_I2p_dv)       _mm_free(_I2p_dv);
    if (_I2p_du_full)  _mm_free(_I2p_du_full);
    if (_I2p_dv_full)  _mm_free(_I2p_dv_full);
    if (_m1c1)         _mm_free(_m1c1);
    if (_m1c2)         _mm_free(_m1c2);
    if (_I1c_du)       _mm_free(_I1c_du);
    if (_I1c_dv)       _mm_free(_I1c_dv);
    if (_I1c_du_full)  _mm_free(_I1c_du_full);
    if (_I1c_dv_full)  _mm_free(_I1c_dv_full);
    if (_m2c1)         _mm_free(_m2c1);
    if (_m2c2)         _mm_free(_m2c2);
    if (_I2c_du)       _mm_free(_I2c_du);
    if (_I2c_dv)       _mm_free(_I2c_dv);
    if (_I2c_du_full)  _mm_free(_I2c_du_full);
    if (_I2c_dv_full)  _mm_free(_I2c_dv_full);
}
...
void Matcher::pushBack(uint8_t *I1,uint8_t* I2,int32_t* dims,const bool replace)
{
    ...
    else
    {
        ...
        if (_I2p_du_full) _mm_free(_I2p_du_full);
        if (_I2p_dv_full) _mm_free(_I2p_dv_full);
        _m1p1 = _m1c1; _n1p1 = _n1c1;
        _m1p2 = _m1c2; _n1p2 = _n1c2;
        _m2p1 = _m2c1; _n2p1 = _n2c1;
        _m2p2 = _m2c2; _n2p2 = _n2c2;
        _I1p         = _I1c;
        _I2p         = _I2c;
        _I1p_du      = _I1c_du;
        _I1p_dv      = _I1c_dv;
        _I1p_du_full = _I1c_du_full;
        _I1p_dv_full = _I1c_dv_full;
        _I2p_du      = _I2c_du;
        _I2p_dv      = _I2c_dv;
        _I2p_du_full = _I2c_du_full;
        _I2p_dv_full = _I2c_dv_full;
        _dims_p[0]   = _dims_c[0];
        _dims_p[1]   = _dims_c[1];
        _dims_p[2]   = _dims_c[2];
    }
    ...
    _dims_c[0] = width;
    _dims_c[1] = height;
    _dims_c[2] = width + 16 - (width) % 16;// width + 15-(width-1)%16;

    // copy images to byte aligned memory
    _I1c = (uint8_t*)_mm_malloc(_dims_c[2]*_dims_c[1]*sizeof(uint8_t),16);
    _I2c = (uint8_t*)_mm_malloc(_dims_c[2]*_dims_c[1]*sizeof(uint8_t),16);
    if (_dims_c[2]==bpl)
    {
        memcpy(_I1c,I1,_dims_c[2]*_dims_c[1]*sizeof(uint8_t));
        if (I2!=0)
        {
            memcpy(_I2c,I2,_dims_c[2]*_dims_c[1]*sizeof(uint8_t));
        }
    }
    else
    {
        ...
        {
            memcpy(_I1c+v*_dims_c[2],I1+v*bpl,_dims_c[0]*sizeof(uint8_t));
            if (I2!=0)
            {
                memcpy(_I2c+v*_dims_c[2],I2+v*bpl,_dims_c[0]*sizeof(uint8_t));
            }
        }
    }
    ...
    if (I2!=0)
    {
        computeFeatures(_I2c,_dims_c,_m2c1,_n2c1,_m2c2,_n2c2,_I2c_du,_I2c_dv,_I2c_du_full,_I2c_dv_full);
    }
}
...
void Matcher::matchFeatures(int32_t method, libviso2_Matrix *Tr_delta)
{
    ...
    if (method==0)
    {
        if (_m1p2==0 || _n1p2==0 || _m1c2==0 || _n1c2==0)
        {
            return;
        }
        if (_param.multi_stage)
        {
            if (_m1p1==0 || _n1p1==0 || _m1c1==0 || _n1c1==0)
            {
                return;
            }
        }
    ...
    {
        if (_m1c2==0 || _n1c2==0 || _m2c2==0 || _n2c2==0)
        {
            return;
        }
        if (_param.multi_stage)
        {
            if (_m1c1==0 || _n1c1==0 || _m2c1==0 || _n2c1==0)
            {
                return;
            }
        }
    ...
    {
        if (_m1p2==0 || _n1p2==0 || _m2p2==0 || _n2p2==0 || _m1c2==0 || _n1c2==0 || _m2c2==0 || _n2c2==0)
        {
            return;
        }
        if (_param.multi_stage)
        {
            if (_m1p1==0 || _n1p1==0 || _m2p1==0 || _n2p1==0 || _m1c1==0 || _n1c1==0 || _m2c1==0 || _n2c1==0)
            {
                return;
            }
        }
    ...
    if (_param.multi_stage)
    {
        ...
        matching(_m1p1,_m2p1,_m1c1,_m2c1,_n1p1,_n2p1,_n1c1,_n2c1,_p_matched_1,method,false,Tr_delta);
        removeOutliers(_p_matched_1,method);

        // compute search range prior statistics (used for speeding up 2nd pass)
        computePriorStatistics(_p_matched_1,method);

        // 2nd pass (dense matches)
        matching(_m1p2,_m2p2,_m1c2,_m2c2,_n1p2,_n2p2,_n1c2,_n2c2,_p_matched_2,method,true,Tr_delta);
        if (_param.refinement>0)
        {
            refinement(_p_matched_2,method);
        }
        removeOutliers(_p_matched_2,method);
    }
    // single pass matching
    else
    {
        matching(_m1p2,_m2p2,_m1c2,_m2c2,_n1p2,_n2p2,_n1c2,_n2c2,_p_matched_2,method,false,Tr_delta);
        if (_param.refinement>0)
        {
            refinement(_p_matched_2,method);
        }
...
void Matcher::bucketFeatures(int32_t max_features,float bucket_width,float bucket_height)
{
    ...
    float u_max = 0;
    float v_max = 0;
    for (vector<p_match>::iterator it = _p_matched_2.begin(); it!=_p_matched_2.end(); it++)
    {
...
float Matcher::getGain(vector<int32_t> inliers)
{
    ...
    if (_I1p==0 || _I1c==0 || _p_matched_2.size()==0 || inliers.size()==0)
    {
        return 1;
    }
    ...
    {
        if (*it<(int32_t)_p_matched_2.size())
        {
            // mean in previous image
            u_min = min(max((int32_t)_p_matched_2[*it].u1p-window_size,0),_dims_p[0]);
            u_max = min(max((int32_t)_p_matched_2[*it].u1p+window_size,0),_dims_p[0]);
            v_min = min(max((int32_t)_p_matched_2[*it].v1p-window_size,0),_dims_p[1]);
            v_max = min(max((int32_t)_p_matched_2[*it].v1p+window_size,0),_dims_p[1]);
            mean_prev = mean(_I1p,_dims_p[2],u_min,u_max,v_min,v_max);

            // mean in current image
            u_min = min(max((int32_t)_p_matched_2[*it].u1c-window_size,0),_dims_p[0]);
            u_max = min(max((int32_t)_p_matched_2[*it].u1c+window_size,0),_dims_p[0]);
            v_min = min(max((int32_t)_p_matched_2[*it].v1c-window_size,0),_dims_p[1]);
            v_max = min(max((int32_t)_p_matched_2[*it].v1c+window_size,0),_dims_p[1]);
            mean_curr = mean(_I1c,_dims_c[2],u_min,u_max,v_min,v_max);
...
void Matcher::nonMaximumSuppression(int16_t* I_f1,int16_t* I_f2,const int32_t* dims,
                                    vector<Matcher::maximum> &maxima,int32_t nms_n)
{
    ...
    for (int32_t i=n+_margin; i<width-n-_margin;i+=n+1)
    {
        for (int32_t j=n+_margin; j<height-n-_margin;j+=n+1)
        {
            ...
            // f1 minimum
            for (int32_t i2=f1mini-n; i2<=min(f1mini+n,width-1-_margin); i2++)
            {
                for (int32_t j2=f1minj-n; j2<=min(f1minj+n,height-1-_margin); j2++)
                {
            ...
            // f1 maximum
            for (int32_t i2=f1maxi-n; i2<=min(f1maxi+n,width-1-_margin); i2++)
            {
                for (int32_t j2=f1maxj-n; j2<=min(f1maxj+n,height-1-_margin); j2++)
                {
                    currval = *(I_f1+getAddressOffsetImage(i2,j2,bpl));
            ...
            // f2 minimum
            for (int32_t i2=f2mini-n; i2<=min(f2mini+n,width-1-_margin); i2++)
            {
                for (int32_t j2=f2minj-n; j2<=min(f2minj+n,height-1-_margin); j2++)
                {
            ...
            // f2 maximum
            for (int32_t i2=f2maxi-n; i2<=min(f2maxi+n,width-1-_margin); i2++)
            {
                for (int32_t j2=f2maxj-n; j2<=min(f2maxj+n,height-1-_margin); j2++)
                {
                    currval = *(I_f2+getAddressOffsetImage(i2,j2,bpl));
...
void Matcher::filterImageAll(uint8_t* I,uint8_t* I_du,uint8_t* I_dv,int16_t* I_f1,
                             int16_t* I_f2,const int* dims)
{
    // get bpl and height
                              ...
                              uint8_t* &I_dv,uint8_t* &I_du_full,uint8_t* &I_dv_full)
{
    ...
    if (_param.multi_stage)
    {
        int32_t nms_n_sparse = _param.nms_n*3;
        if (nms_n_sparse>10)
        {
...
void Matcher::computePriorStatistics(vector<Matcher::p_match> &p_matched,int32_t method)
{
    ...
    for (vector<Matcher::p_match>::iterator it=p_matched.begin(); it!=p_matched.end(); it++)
    {
        ...
        if (method<2)
        {
            u_bin_min = min(max((int32_t)floor(it->u1c/(float)_param.match_binsize)-1,0),u_bin_num-1);
            u_bin_max = min(max((int32_t)floor(it->u1c/(float)_param.match_binsize)+1,0),u_bin_num-1);
            v_bin_min = min(max((int32_t)floor(it->v1c/(float)_param.match_binsize)-1,0),v_bin_num-1);
            v_bin_max = min(max((int32_t)floor(it->v1c/(float)_param.match_binsize)+1,0),v_bin_num-1);
        }
        // quad matching: use current previous image as reference
        else
        {
            u_bin_min = min(max((int32_t)floor(it->u1p/(float)_param.match_binsize)-1,0),u_bin_num-1);
            u_bin_max = min(max((int32_t)floor(it->u1p/(float)_param.match_binsize)+1,0),u_bin_num-1);
            v_bin_min = min(max((int32_t)floor(it->v1p/(float)_param.match_binsize)-1,0),v_bin_num-1);
            v_bin_max = min(max((int32_t)floor(it->v1p/(float)_param.match_binsize)+1,0),v_bin_num-1);
        }
    ...
    for (int32_t v_bin=0; v_bin<v_bin_num; v_bin++)
    {
        for (int32_t u_bin=0; u_bin<u_bin_num; u_bin++)
        {
            ...
            delta delta_min(-_param.match_radius);
            delta delta_max(+_param.match_radius);
                                ...
                                const int32_t &u_bin_num,const int32_t &v_bin_num)
{
    ...
    for (int32_t i=0; i<n; i++)
    {
        ...
        // compute row and column of bin to which this observation belongs
        int32_t u_bin = min((int32_t)floor((float)u/(float)_param.match_binsize),u_bin_num-1);
        int32_t v_bin = min((int32_t)floor((float)v/(float)_param.match_binsize),v_bin_num-1);
                               ...
                               int32_t& min_ind,int32_t stage,bool flow,bool use_prior,double u_,double v_)
{
    ...
    if (use_prior)
    {
        u_min = u1+_ranges[stat_bin].u_min[stage];
        u_max = u1+_ranges[stat_bin].u_max[stage];
        v_min = v1+_ranges[stat_bin].v_min[stage];
        v_max = v1+_ranges[stat_bin].v_max[stage];
    }
    // otherwise: use full search space
    else
    {
        u_min = u1-_param.match_radius;
        u_max = u1+_param.match_radius;
        v_min = v1-_param.match_radius;
        v_max = v1+_param.match_radius;
    }

    // if stereo search => constrain to 1d
    if (!flow)
    {
        v_min = v1-_param.match_disp_tolerance;
        v_max = v1+_param.match_disp_tolerance;
    }

    // bins of interest
    int32_t u_bin_min = min(max((int32_t)floor(u_min/(float)_param.match_binsize),0),u_bin_num-1);
    int32_t u_bin_max = min(max((int32_t)floor(u_max/(float)_param.match_binsize),0),u_bin_num-1);
                       ...
                       vector<Matcher::p_match> &p_matched,int32_t method,bool use_prior,libviso2_Matrix *Tr_delta)
{
    ...
    // loop variables
    int32_t* M = (int32_t*)calloc(_dims_c[0]*_dims_c[1],sizeof(int32_t));
    ...
    if (Tr_delta)
    {
        ...
        t02 = Tr_delta->_val[0][2];
        t03 = Tr_delta->_val[0][3];
        t10 = Tr_delta->_val[1][0];
        t11 = Tr_delta->_val[1][1];
        t12 = Tr_delta->_val[1][2];
        t13 = Tr_delta->_val[1][3];
        t20 = Tr_delta->_val[2][0];
        t21 = Tr_delta->_val[2][1];
        t22 = Tr_delta->_val[2][2];
        t23 = Tr_delta->_val[2][3];
    }
    ...
    if (method==0)
    {
        ...
        for (i1c=0; i1c<n1c; i1c++)
        {
            ...
            int32_t u_bin = min((int32_t)floor((float)u1c/(float)_param.match_binsize),u_bin_num-1);
            int32_t v_bin = min((int32_t)floor((float)v1c/(float)_param.match_binsize),v_bin_num-1);
            ...
            if (i1c2==i1c)
            {
                ...
                if (*(M+getAddressOffsetImage(u1c,v1c,_dims_c[0]))==0)
                {
                    p_matched.push_back(Matcher::p_match(u1p,v1p,i1p,-1,-1,-1,u1c,v1c,i1c,-1,-1,-1));
                    *(M+getAddressOffsetImage(u1c,v1c,_dims_c[0])) = 1;
                }
            }
        }
    }
    ...
    else
    {
        ...
        for (i1p=0; i1p<n1p; i1p++)
        {
            ...
            // compute row and column of statistics bin to which this observation belongs
            int32_t u_bin = min((int32_t)floor((float)u1p/(float)_param.match_binsize),u_bin_num-1);
            ...
            if (Tr_delta)
            {
                ...
                double x1p = ((double)u1p-_param.cu)*_param.base/d;
                double y1p = ((double)v1p-_param.cv)*_param.base/d;
                double z1p = _param.f*_param.base/d;

                double x2c = t00*x1p + t01*y1p + t02*z1p + t03 - _param.base;
...
void Matcher::removeOutliers(vector<Matcher::p_match> &p_matched,int32_t method)
{
    ...
    for (int32_t i=0; i<out.numberoftriangles; i++)
    {
        ...
        if (method==0)
        {
            ...
            if (fabs(p1_flow_u-p2_flow_u)+fabs(p1_flow_v-p2_flow_v)<_param.outlier_flow_tolerance)
            {
                num_support[p1]++;
                num_support[p2]++;
            }
            ...
            if (fabs(p2_flow_u-p3_flow_u)+fabs(p2_flow_v-p3_flow_v)<_param.outlier_flow_tolerance)
            {
                num_support[p2]++;
                num_support[p3]++;
            }
            ...
            if (fabs(p1_flow_u-p3_flow_u)+fabs(p1_flow_v-p3_flow_v)<_param.outlier_flow_tolerance)
            {
                num_support[p1]++;
                num_support[p3]++;
            }
        ...
        else if (method==1)
        {
            ...
            // consistency of 1. edge
            if (fabs(p1_disp-p2_disp)<_param.outlier_disp_tolerance)
            {
                num_support[p1]++;
                num_support[p2]++;
            }
            ...
            // consistency of 2. edge
            if (fabs(p2_disp-p3_disp)<_param.outlier_disp_tolerance)
            {
                num_support[p2]++;
                num_support[p3]++;
            }
            ...
            // consistency of 3. edge
            if (fabs(p1_disp-p3_disp)<_param.outlier_disp_tolerance)
            {
                num_support[p1]++;
                num_support[p3]++;
            }
        ...
        else
        {
            ...
            // consistency of 1. edge
            if (fabs(p1_disp-p2_disp)<_param.outlier_disp_tolerance &&
                fabs(p1_flow_u-p2_flow_u)+fabs(p1_flow_v-p2_flow_v)<_param.outlier_flow_tolerance)
            {
                num_support[p1]++;
                num_support[p2]++;
            }
            ...
            // consistency of 2. edge
            if (fabs(p2_disp-p3_disp)<_param.outlier_disp_tolerance &&
                fabs(p2_flow_u-p3_flow_u)+fabs(p2_flow_v-p3_flow_v)<_param.outlier_flow_tolerance)
            {
                num_support[p2]++;
                num_support[p3]++;
            }
            ...
            // consistency of 3. edge
            if (fabs(p1_disp-p3_disp)<_param.outlier_disp_tolerance &&
                fabs(p1_flow_u-p3_flow_u)+fabs(p1_flow_v-p3_flow_v)<_param.outlier_flow_tolerance)
            {
                num_support[p1]++;
                num_support[p3]++;
            }
                               ...
                               uint8_t* desc_buffer)
{
    ...
    if (u2-3<_margin || u2+3>dims2[0]-1-_margin || v2-3<_margin || v2+3>dims2[1]-1-_margin)
    {
        return false;
    }
    ...
    float divisor = (b._val[2][0]*b._val[2][0]-4.0*b._val[0][0]*b._val[1][0]);
    if (fabs(divisor)<1e-8 || fabs(b._val[2][0])<1e-8)
    {
        return false;
    }
    float ddv = (2.0*b._val[0][0]*b._val[4][0]-b._val[2][0]*b._val[3][0])/divisor;
    float ddu = -(b._val[4][0]+2.0*b._val[1][0]*ddv)/b._val[2][0];
...
{
    // check if parabolic fitting is feasible (descriptors are within _margin)
    if (u2-2<_margin || u2+2>dims2[0]-1-_margin || v2-2<_margin || v2+2>dims2[1]-1-_margin)
    {
        return;
    }
...
void Matcher::refinement(vector<Matcher::p_match> &p_matched,int32_t method)
{
    ...
    uint8_t* I2p_du_fit = _I2p_du;
    uint8_t* I2p_dv_fit = _I2p_dv;
    uint8_t* I1c_du_fit = _I1c_du;
    uint8_t* I1c_dv_fit = _I1c_dv;
    uint8_t* I2c_du_fit = _I2c_du;
    uint8_t* I2c_dv_fit = _I2c_dv;
    if (_param.half_resolution)
    {
        I1p_du_fit = _I1p_du_full;
        I1p_dv_fit = _I1p_dv_full;
        I2p_du_fit = _I2p_du_full;
        I2p_dv_fit = _I2p_dv_full;
        I1c_du_fit = _I1c_du_full;
        I1c_dv_fit = _I1c_dv_full;
        I2c_du_fit = _I2c_du_full;
        I2c_dv_fit = _I2c_dv_full;
    }
    ...
    for (vector<Matcher::p_match>::iterator it=p_matched_copy.begin(); it!=p_matched_copy.end(); it++)
    {
        ...
        {
            if (_param.refinement==2)
            {
                if (!parabolicFitting(I1c_du_fit,I1c_dv_fit,_dims_c,I1p_du_fit,I1p_dv_fit,_dims_p,
            ...
            {
                relocateMinimum(I1c_du_fit,I1c_dv_fit,_dims_c,I1p_du_fit,I1p_dv_fit,_dims_p,
        ...
        if (method==1 || method==2)
        {
            if (_param.refinement==2)
            {
                if (!parabolicFitting(I1c_du_fit,I1c_dv_fit,_dims_c,I2c_du_fit,I2c_dv_fit,_dims_c,
            ...
            {
                relocateMinimum(I1c_du_fit,I1c_dv_fit,_dims_c,I2c_du_fit,I2c_dv_fit,_dims_c,
        ...
        if (method==2)
        {
            if (_param.refinement==2)
            {
                if (!parabolicFitting(I1c_du_fit,I1c_dv_fit,_dims_c,I2p_du_fit,I2p_dv_fit,_dims_p,
            ...
            {
                relocateMinimum(I1c_du_fit,I1c_dv_fit,_dims_c,I2p_du_fit,I2p_dv_fit,_dims_p,
...
float Matcher::mean(const uint8_t* I,const int32_t &bpl,const int32_t &u_min,
                    const int32_t &u_max,const int32_t &v_min,const int32_t &v_max)
{
    float mean = 0;
