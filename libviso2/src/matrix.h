...
public:
    ...
    libviso2_Matrix(const int32_t m,const int32_t n,const FLOAT* val); // init mxn libviso2_Matrix with values from array 'val'
    ...
    void getData(FLOAT* val,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
    ...
    bool   lu(int32_t *idx, FLOAT &d);        // replace *this by lower upper decomposition
...
private:
    ...
    void allocateMemory(const int32_t m,const int32_t n);
    void releaseMemory();
