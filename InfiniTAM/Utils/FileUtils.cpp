...
{   
    ...
    if( disparityFromFile )
    {
        ...
        xsize = StereoImage::m_currentFrame->width;
        ysize = StereoImage::m_currentFrame->height;
        short *data = new short[xsize*ysize];
        Vector2i newSize(xsize, ysize);
        image->ChangeDims(newSize);
        
        floatToShort(StereoImage::m_currentFrame->D1, data);
...
{
    int cols = StereoImage::m_currentFrame->width;
    int rows = StereoImage::m_currentFrame->height;
    float rawDisparity;
