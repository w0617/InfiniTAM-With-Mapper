...
{
    // get _width, _height and bytes per line
    _width  = dims[0];
    _height = dims[1];
    _bpl    = _width + 15-(_width-1)%16;

    // copy images to byte aligned memory
    _I1 = (uint8_t*)_mm_malloc(_bpl*_height*sizeof(uint8_t),16);
    _I2 = (uint8_t*)_mm_malloc(_bpl*_height*sizeof(uint8_t),16);
    memset (_I1,0,_bpl*_height*sizeof(uint8_t));
    memset (_I2,0,_bpl*_height*sizeof(uint8_t));
    if (_bpl==dims[2])
    {
        memcpy(_I1,I1_,_bpl*_height*sizeof(uint8_t));
        memcpy(_I2,I2_,_bpl*_height*sizeof(uint8_t));
    }
    else
    {
        for (int32_t v=0; v<_height; v++)
        {
            memcpy(_I1+v*_bpl,I1_+v*dims[2],_width*sizeof(uint8_t));
            memcpy(_I2+v*_bpl,I2_+v*dims[2],_width*sizeof(uint8_t));
        }
    }
...
#endif
    Descriptor desc1(_I1,_width,_height,_bpl,_param.subsampling);
    Descriptor desc2(_I2,_width,_height,_bpl,_param.subsampling);
#ifdef PROFILE
    timer.start("Support Matches");
#endif
    vector<support_pt> p_support = computeSupportMatches(desc1._I_desc,desc2._I_desc);
...
#endif
    computeDisparity(p_support,tri_1,disparity_grid_1,grid_dims,desc1._I_desc,desc2._I_desc,0,D1);
    computeDisparity(p_support,tri_2,disparity_grid_2,grid_dims,desc1._I_desc,desc2._I_desc,1,D2);
    ...
    if (!_param.postprocess_only_left)
    {
    ...
    if (_param.filter_adaptive_mean)
    {
#ifdef PROFILE
    ...
    for (int32_t u_can=0; u_can<D_can_width; u_can++)
    {
        for (int32_t v_can=0; v_can<D_can_height; v_can++)
        {
            ...
            if (d_can>=0)
            {
                ...
                int32_t support = 0;
                for (int32_t u_can_2=u_can-_param.incon_window_size; u_can_2<=u_can+_param.incon_window_size; u_can_2++)
                {
                    for (int32_t v_can_2=v_can-_param.incon_window_size; v_can_2<=v_can+_param.incon_window_size; v_can_2++)
                    {
                        if (u_can_2>=0 && v_can_2>=0 && u_can_2<D_can_width && v_can_2<D_can_height)
                        {
                            ...
                            if (d_can_2>=0 && abs(d_can-d_can_2)<=_param.incon_threshold)
                            {
                                support++;
                            }
                        }
                ...
                if (support<_param.incon_min_support)
                {
...
void Elas::addCornerSupportPoints(vector<support_pt> &p_support)
{
    ...
    p_border.push_back(support_pt(0,_height-1,0));
    p_border.push_back(support_pt(_width-1,0,0));
    p_border.push_back(support_pt(_width-1,_height-1,0));
...
inline int16_t Elas::computeMatchingDisparity(const int32_t &u,const int32_t &v,
                                              uint8_t* I1_desc,uint8_t* I2_desc,const bool &right_image)
{
    ...
    int32_t desc_offset_1 = -16*u_step-16*_width*v_step;
    int32_t desc_offset_2 = +16*u_step-16*_width*v_step;
    int32_t desc_offset_3 = -16*u_step+16*_width*v_step;
    int32_t desc_offset_4 = +16*u_step+16*_width*v_step;

    __m128i xmm1,xmm2,xmm3,xmm4,xmm5,xmm6;

    // check if we are inside the image region
    if (u>=window_size+u_step && u<=_width-window_size-1-u_step && v>=window_size+v_step && v<=_height-window_size-1-v_step)
    {
        // compute desc and start addresses
        int32_t  line_offset = 16*_width*v;
        ...
        // compute _I1 block start addresses
        ...
        int32_t disp_min_valid = max(_param.disp_min,0);
        int32_t disp_max_valid = _param.disp_max;
        if (!right_image) disp_max_valid = min(_param.disp_max,u-window_size-u_step);
        else              disp_max_valid = min(_param.disp_max,_width-u-window_size-u_step);
        ...
        if (min_1_d>=0 && min_2_d>=0 && (float)min_1_E<_param.support_threshold*(float)min_2_E)
        {
            return min_1_d;
        }
        else
...
vector<Elas::support_pt> Elas::computeSupportMatches(uint8_t* I1_desc,uint8_t* I2_desc)
{
    ...
    int32_t D_candidate_stepsize = _param.candidate_stepsize;
    ...
    for (int32_t v=0; v<_height; v+=D_candidate_stepsize) D_can_height++;
        ...
        for (int32_t v_can=1; v_can<D_can_height; v_can++)
        {
            ...
            *(D_can+getAddressOffsetImage(u_can,v_can,D_can_width)) = -1;
            ...
            if (d>=0)
            {
                ...
                if (d2>=0 && abs(d-d2)<=_param.lr_threshold)
                {
...
void Elas::createGrid(vector<support_pt> p_support,int32_t* disparity_grid,int32_t* grid_dims,bool right_image)
{
    ...
    for (vector<support_pt>::size_type i=0; i<p_support.size(); i++)
    {
        ...
        for (int32_t d=d_min; d<=d_max; d++)
        {
            ...
            {
                x = floor((float)(x_curr-d_curr)/(float)_param.grid_size);
            }
    ...
    const int32_t* cl = temp1 + (1*grid_width+0)*(_param.disp_max+1);
    const int32_t* cc = temp1 + (1*grid_width+1)*(_param.disp_max+1);
    const int32_t* cr = temp1 + (1*grid_width+2)*(_param.disp_max+1);
    const int32_t* bl = temp1 + (2*grid_width+0)*(_param.disp_max+1);
    const int32_t* bc = temp1 + (2*grid_width+1)*(_param.disp_max+1);
    const int32_t* br = temp1 + (2*grid_width+2)*(_param.disp_max+1);

    int32_t* result    = temp2 + (1*grid_width+1)*(_param.disp_max+1);
    ...
    for (int32_t x=0; x<grid_width; x++)
    {
        for (int32_t y=0; y<grid_height; y++)
        {
            ...
            *(disparity_grid+getAddressOffsetGrid(x,y,0,grid_width,_param.disp_max+2))=curr_ind-1;
        }
    }
                            ...
                            int32_t *P,int32_t &plane_radius,bool &valid,bool &right_image,float* D)
{
    ...
    uint32_t d_addr;
    if (_param.subsampling) d_addr = getAddressOffsetImage(u/2,v/2,_width/2);
    else                   d_addr = getAddressOffsetImage(u,v,_width);

    // check if u is ok
    if (u<window_size || u>=_width-window_size)
    {
        return;
    }
    ...
    // compute _I1 block start address
    uint8_t* I1_block_addr = I1_line_addr+16*u;
    ...
    if (!right_image)
    {
        for (int32_t i=0; i<num_grid; i++)
        {
            ...
            {
                u_warp = u-d_curr;
                if (u_warp<window_size || u_warp>=_width-window_size)
                {
                    continue;
                }
                updatePosteriorMinimum((__m128i*)(I2_line_addr+16*u_warp),d_curr,
                                       xmm1,xmm2,val,min_val,min_d);
            }
        }
        for (d_curr=d_plane_min; d_curr<=d_plane_max; d_curr++)
        {
            ...
            updatePosteriorMinimum((__m128i*)(I2_line_addr+16*u_warp),d_curr,
                                   valid?*(P+abs(d_curr-d_plane)):0,xmm1,xmm2,val,min_val,min_d);
        }
                            ...
                            uint8_t* I1_desc,uint8_t* I2_desc,bool right_image,float* D)
{
    ...
    if (_param.subsampling)
    {
        for (int32_t i=0; i<(_width/2)*(_height/2); i++)
        {
            *(D+i) = -10;
        }
    }
    ...
    for (uint32_t i=0; i<tri.size(); i++)
    {
        ...
        {
            for (int32_t u=max((int32_t)B_u,0); u<min((int32_t)C_u,_width); u++)
            {
                if (!_param.subsampling || u%2==0)
                {
                    int32_t v_1 = (uint32_t)(AC_a*(float)u+AC_b);
...
void Elas::leftRightConsistencyCheck(float* D1,float* D2)
{
    ...
    int32_t D_width  = _width;
    int32_t D_height = _height;
    if (_param.subsampling)
    {
        D_width  = _width/2;
        D_height = _height/2;
    ...
    for (int32_t u=0; u<D_width; u++)
    {
        for (int32_t v=0; v<D_height; v++)
        {
            ...
            if (d1>=0 && u_warp_1>=0 && u_warp_1<D_width)
            {
                ...
                // if check failed
                if (fabs(*(D2_copy+addr_warp)-d1)>_param.lr_threshold)
                {
                    *(D1+addr) = -10;
                }
            }
            ...
            if (d2>=0 && u_warp_2>=0 && u_warp_2<D_width)
            {
                ...
                // if check failed
                if (fabs(*(D1_copy+addr_warp)-d2)>_param.lr_threshold)
                {
                    *(D2+addr) = -10;
                }
            }
...
void Elas::removeSmallSegments(float* D)
{
    ...
    int32_t D_width        = _width;
    int32_t D_height       = _height;
    int32_t D_speckle_size = _param.speckle_size;
    if (_param.subsampling)
    {
        D_width        = _width/2;
        D_height       = _height/2;
        D_speckle_size = sqrt((float)_param.speckle_size)*2;
    }
                    ...
                    for (int32_t i=0; i<4; i++)
                    {
                        ...
                        if (u_neighbor[i]>=0 && v_neighbor[i]>=0 && u_neighbor[i]<D_width && v_neighbor[i]<D_height)
                        {
                            ...
                            if (*(D_done+addr_neighbor)==0 && *(D+addr_neighbor)>=0)
                            {
                                ...
                                if (fabs(*(D+addr_curr)-*(D+addr_neighbor))<=_param.speckle_sim_threshold)
                                {
...
void Elas::gapInterpolation(float* D)
{
    ...
    int32_t D_width          = _width;
    int32_t D_height         = _height;
    int32_t D_ipol_gap_width = _param.ipol_gap_width;
    if (_param.subsampling)
    {
        D_width          = _width/2;
        D_height         = _height/2;
        D_ipol_gap_width = _param.ipol_gap_width/2+1;
    }
...
void Elas::adaptiveMean(float* D)
{
    ...
    // when doing subsampling: 4 pixel bilateral filter _width
    if (_param.subsampling)
    {
        // horizontal filter
    ...
    // full resolution: 8 pixel bilateral filter _width
    else
    {
        // horizontal filter
        for (int32_t v=3; v<D_height-3; v++)
