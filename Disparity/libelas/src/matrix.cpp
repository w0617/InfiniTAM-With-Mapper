...
libviso2_Matrix::libviso2_Matrix (const int32_t m_,const int32_t n_) {
  allocateMemory(m_,n_);
}

libviso2_Matrix::libviso2_Matrix (const int32_t m_,const int32_t n_,const FLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
libviso2_Matrix libviso2_Matrix::getMat(int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  ...
  if (i1<0 || i2>=m || j1<0 || j2>=n || i2<i1 || j2<j1) {
    cerr << "ERROR: Cannot get sublibviso2_Matrix [" << i1 << ".." << i2 <<
        "] x [" << j1 << ".." << j2 << "]" <<
        " of a (" << m << "x" << n << ") libviso2_Matrix." << endl;
    exit(0);
  }
...
void libviso2_Matrix::setMat(const libviso2_Matrix &M,const int32_t i1,const int32_t j1) {
  if (i1<0 || j1<0 || i1+M.m>m || j1+M.n>n) {
    cerr << "ERROR: Cannot set sublibviso2_Matrix [" << i1 << ".." << i1+M.m-1 <<
        "] x [" << j1 << ".." << j1+M.n-1 << "]" <<
        " of a (" << m << "x" << n << ") libviso2_Matrix." << endl;
    exit(0);
  }
...
libviso2_Matrix libviso2_Matrix::eye (const int32_t m) {
  libviso2_Matrix M(m,m);
  for (int32_t i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
}
...
libviso2_Matrix libviso2_Matrix::diag (const libviso2_Matrix &M) {
  if (M.m>1 && M.n==1) {
    libviso2_Matrix D(M.m,M.m);
    for (int32_t i=0; i<M.m; i++)
      D.val[i][i] = M.val[i][0];
    return D;
  } else if (M.m==1 && M.n>1) {
    libviso2_Matrix D(M.n,M.n);
    for (int32_t i=0; i<M.n; i++)
      D.val[i][i] = M.val[0][i];
    return D;
  }
  cout << "ERROR: Trying to create diagonal libviso2_Matrix from vector of size (" << M.m << "x" << M.n << ")" << endl;
  exit(0);
}

libviso2_Matrix libviso2_Matrix::reshape(const libviso2_Matrix &M,int32_t m_,int32_t n_) {
  if (M.m*M.n != m_*n_) {
    cerr << "ERROR: Trying to reshape a libviso2_Matrix of size (" << M.m << "x" << M.n <<
...
libviso2_Matrix libviso2_Matrix::inv (const libviso2_Matrix &M) {
  if (M.m!=M.n) {
    cerr << "ERROR: Trying to invert libviso2_Matrix of size (" << M.m << "x" << M.n << ")" << endl;
    exit(0);
  }
  libviso2_Matrix A(M);
  libviso2_Matrix B = eye(M.m);
  B.solve(A);
  return B;
}
...
bool libviso2_Matrix::inv () {
  if (m!=n) {
    cerr << "ERROR: Trying to invert libviso2_Matrix of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
FLOAT libviso2_Matrix::det () {
  ...
  if (m != n) {
    cerr << "ERROR: Trying to compute determinant of a libviso2_Matrix of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
// Given a libviso2_Matrix a[1..n][1..n], this routine replaces it by the LU decomposition of a rowwise
...
bool libviso2_Matrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
  ...
  if (m != n) {
    cerr << "ERROR: Trying to LU decompose a libviso2_Matrix of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
void libviso2_Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
